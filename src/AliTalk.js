...
const AliTalk = () => {
  ...
  const aliTalkWorkshopName = "Ali Talk";
  ...
  const handleFileUpload = (event) => {
    ...
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        let lastProcessedRow = localStorage.getItem("lastProcessedRowAli");
  ...
  const sendBulkEmails = async () => {
    ...
    if (maxRowNumber > 0) {
      ...
      localStorage.setItem("lastProcessedRowAli", maxRowNumber);
    }
