...
const VineetTalk = () => {
  ...
  const aliTalkWorkshopName = "Vineet Talk";
  ...
  const handleFileUpload = (event) => {
    ...
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        let lastProcessedRow = localStorage.getItem("lastProcessedRowVineet");
  ...
  const sendBulkEmails = async () => {
    ...
    if (maxRowNumber > 0) {
      ...
      localStorage.setItem("lastProcessedRowVineet", maxRowNumber);
    }
