...
const Ishrae = () => {
  ...
  const aliTalkWorkshopName = "Ishrae Talk";
  ...
  const handleFileUpload = (event) => {
    ...
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        let lastProcessedRow = localStorage.getItem("lastProcessedRowIshrae");
  ...
  const sendBulkEmails = async () => {
    ...
    if (maxRowNumber > 0) {
      ...
      localStorage.setItem("lastProcessedRowIshrae", maxRowNumber);
    }
