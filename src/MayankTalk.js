...
const MayankTalk = () => {
  ...
  const aliTalkWorkshopName = "Mayank Talk";
  ...
  const handleFileUpload = (event) => {
    ...
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        let lastProcessedRow = localStorage.getItem("lastProcessedRowMayank");
  ...
  const sendBulkEmails = async () => {
    ...
    if (maxRowNumber > 0) {
      ...
      localStorage.setItem("lastProcessedRowMayank", maxRowNumber);
    }
